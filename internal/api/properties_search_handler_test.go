...
func TestSearchPropertiesSerializesDTOInSnakeCase(t *testing.T) {
	...
	// Os nomes vêm de propertyResponse, o DTO compartilhado com o detalhe. O
	// item da busca só acrescenta active_listing_count.
	want := []string{
		"id", "transaction_type", "property_type", "bedrooms", "bathrooms",
		"parking_spots", "area", "amenities", "canonical_address", "city",
		"neighborhood", "state", "photos", "description", "latitude", "longitude",
		"created_at", "updated_at", "active_listing_count",
	}
	item := payload.Data[0]
	...
	if item["active_listing_count"] != float64(2) {
		t.Errorf("active_listing_count = %v, want 2", item["active_listing_count"])
	}
	if item["created_at"] != "2026-03-04T05:06:07Z" {
		t.Errorf("created_at = %v, want RFC 3339 em UTC", item["created_at"])
	}
}

// TestSearchAndDetailShareThePropertyDTO trava a decisão registrada em
// internal/api/CLAUDE.md: um único mapeamento JSON para o imóvel. Dois structs
// divergiriam no primeiro campo novo e o front trataria "o mesmo imóvel" de dois
// jeitos entre a listagem e o detalhe.
func TestSearchAndDetailShareThePropertyDTO(t *testing.T) {
	property := db.Property{ID: "abc", City: ptr("Curitiba"), BedroomCount: ptr(3)}

	fromSearch, err := json.Marshal(propertyListItem{
		propertyResponse: newPropertyResponse(property),
	})
	if err != nil {
		t.Fatalf("marshal do item da busca: %v", err)
	}
	fromDetail, err := json.Marshal(newPropertyDetailResponse(db.PropertyDetail{Property: property}))
	if err != nil {
		t.Fatalf("marshal do detalhe: %v", err)
	}

	var searchFields, detailFields map[string]any
	if err := json.Unmarshal(fromSearch, &searchFields); err != nil {
		t.Fatalf("unmarshal do item da busca: %v", err)
	}
	if err := json.Unmarshal(fromDetail, &detailFields); err != nil {
		t.Fatalf("unmarshal do detalhe: %v", err)
	}

	for field, want := range detailFields {
		if field == "listings" {
			continue
		}
		got, ok := searchFields[field]
		if !ok {
			t.Errorf("campo %q existe no detalhe e sumiu da busca", field)
			continue
		}
		if fmt.Sprint(got) != fmt.Sprint(want) {
			t.Errorf("campo %q = %v na busca e %v no detalhe", field, got, want)
		}
	}
}
...
func TestSearchPropertiesSerializesNilsAsNullAndSlicesAsEmpty(t *testing.T) {
	...
	for _, fragment := range []string{`"canonical_address":null`, `"bedrooms":null`, `"latitude":null`} {
