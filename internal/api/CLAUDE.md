...
- **Ordem da cadeia, de fora para dentro: `recovery → requestID → logging →
  cors → rateLimit → jsonErrors → mux`.** Cada posição é deliberada:
...
- **`rateLimitExemptPaths` (`/health`, `/metrics`) não toca o Redis nem emite os
  headers.** Liveness do orquestrador não pode receber 429 por causa do tráfego
  de um vizinho no mesmo NAT. `/metrics` já está no conjunto sem a rota existir,
